...
class PositionController {
  ...
  async addPosition(ctx: ParameterizedContext, next: Next) {
    ...
    const maxValue = Math.max(...positionList.map((item) => item.value));

    await positionServices.addPosition(
      { ...(ctx.request.body ?? {}), value: maxValue + 1 } as PositionOrderResultModel,
      ctx.userInfo
    );
    ctx.success({
      message: '新建仓位成功',
  ...
  async editPosition(ctx: ParameterizedContext, next: Next) {
    ...
    const oldPosition = await positionServices.getPositionById(ctx.request.body as PositionOrderResultModel);
    ...
    await positionServices.editPosition(
      _.pick(ctx.request.body, ['label', 'description', 'id']) as PositionOrderResultModel,
      ctx.userInfo
    );
    ctx.success({
      message: '编辑成功',
