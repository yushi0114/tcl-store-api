...
import type { ILogin, IUserInfo, Page, ChangePasswordParams } from '../../global/types';
...
class UserController {
  ...
  async accountExist(ctx: ParameterizedContext, next: Next) {
    // 判断用户名是否存在
    const res = (await userServices.loginUser(
      ctx.request.body as ILogin
    )) as Pick<IUserInfo, 'userId' | 'username' | 'password'>[];
  ...
  async deleteUser(ctx: ParameterizedContext, next: Next) {
    const idObj = ctx.request.body as IUserInfo;
  ...
  // 修改密码
  async changePassword(ctx: ParameterizedContext, next: Next) {
    const { userId, password } = ctx.userInfo;
    const { passwordNew, passwordOld } = ctx.request.body as ChangePasswordParams;
    if (passwordOld != password) {
      ctx.app.emit('error', ErrorTypes.OLD_PASSWORD_ERROR, ctx);
      return;
    }
    const res = await resetUser(userId!, passwordNew);
