...
class IncomeServices {
  ...
  editIncome(data: IncomingOrderResultModel, userInfo: IUserInfo) {
    const { userId } = userInfo;
    const res = mysql.actionUpdate('incoming_order', { updateBy: userId, updateTime: formatToDateTime(), ...data}, ['id', data.id!]);
    return res;
  }
  ...
  async getPositionList() {
    const res = await mysql.actionQuery('position_good') as LabelValueOptions<number>;
    return res;
  }

  // 获取仓库
  async getTypeList() {
    const res = await mysql.actionQuery('good_type') as LabelValueOptions<number>;
    return res;
  }
}
